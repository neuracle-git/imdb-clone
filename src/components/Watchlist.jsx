...
function Watchlist({watchList, removeFromWatchList}) {
    ...
    useEffect(() => {
        console.log("I am called",watchList);
        
        let tmp = watchList.map((movieObj) => {
            console.log(movieObj,GenreList[movieObj.genre_ids[0]]);
            
            return GenreList[movieObj.genre_ids[0]]
        })
        setwatchGenreList([...new Set(['All Genre',...tmp])])
        console.log("hola",watchGenreList);
        
    },[watchList])

    console.log("h",watchList);
  ...
  return (
    <div className="overflow-x-auto">
     ...
     <div className="flex flex-row gap-4 mt-8 mb-8 justify-center">
        ...
        return (
                <button key={idx} onClick={() => setSelectedGenre(genre)}  className={selectedGenre != genre?"btn btn-gray w-24" :"btn btn-primary w-24"}>
                {genre}
                </button>
        ) 
        })
    ...
    <div className="mt-8 mb-8 text-center">
        ...
        <input type="text" placeholder="Search" className="input input-bordered w-auto" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
     </div>
      ...
      <table className="table">
        ...
        <tbody>
          ...
          { watchList.filter((movieObj) => {
            ...
            return (movieObj.original_title).toLowerCase().includes(searchText.toLowerCase())
          }).map((movieObj,idx) =>{
                console.log("hello",idx);
                    ...
                    return (
                        ...
                        <td>
                            <button className="bg bg-warning" onClick={() => removeFromWatchList(movieObj)}>Delete</button>
                        </td>
                      </tr>        
                    )
                }
